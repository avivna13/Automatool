#!/usr/bin/env python3
"""
Test suite for TTF Font Steganography Detection

Tests the detect_ttf_steganography module with various scenarios including
the myfont.ttf file from resources (8.1 MB - should be flagged as suspicious).
"""
...
from detect_ttf_steganography import detect_ttf_steganography, _generate_simple_font_report


class TestTTFSteganographyDetection(unittest.TestCase):
    """Test suite for TTF font steganography detection."""
    ...
    def test_07_error_handling(self):
        ...
        result = detect_ttf_steganography(self.test_output_dir, self.test_output_dir, verbose=False)
        self.assertIsNone(result, "Directory should return None")
        
        # Test with None path
        result = detect_ttf_steganography(None, self.test_output_dir, verbose=False)
        self.assertIsNone(result, "None path should return None")
        
        print(f"✅ Error handling working correctly")
    
    def test_08_performance_characteristics(self):
    ...
    def test_09_memory_usage(self):
        """Test that memory usage is minimal (no file content loading)."""
        ...
        self.assertLess(memory_increase, 1024 * 1024, 
                       f"Memory increase should be minimal, increased by {memory_increase/1024:.1f} KB")
        
        print(f"✅ Memory usage test passed: {memory_increase/1024:.1f} KB increase (under 1MB threshold)")


def run_tests():
    """Run the test suite."""
    print("🧪 Starting TTF Font Steganography Detection Tests")
    print("=" * 60)
    
    # Create test suite
    suite = unittest.TestLoader().loadTestsFromTestCase(TestTTFSteganographyDetection)
