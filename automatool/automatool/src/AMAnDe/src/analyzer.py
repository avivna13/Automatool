...
class Analyzer:
    ...
    def analyzeAccessibilityServices(self):
        ...
        if len(accessibility_services) == 0:
            print("No accessibility services found")
        ...
        for service in accessibility_services:
            ...
            # Security warnings - use print() to ensure they appear in output file
            print(colored(f'[-] Accessibility service found: {service_name}', "red"))
            
            if service.exported:
                print(colored(f'[!] Accessibility service {service_name} is exported - HIGH RISK', "red", attrs=['bold']))
            
            if service.enabled is not False:  # None or True
                print(colored(f'[-] Accessibility service {service_name} is enabled by default', "red"))
        
        self.json_result["Accessibility Services"] = jres
        
        # Summary warning - use print() to ensure it appears in output file
        if len(accessibility_services) > 0:
            print(colored(f'[-] Found {len(accessibility_services)} accessibility service(s). '
                         f'These services have extensive system access and should be carefully reviewed.', "red"))
    ...
    def analyzeNotificationListenerServices(self):
        """
        Analyzes notification listener services and their security implications.
        
        Security concerns:
        - Notification listeners can read all system notifications
        - Access to sensitive information (2FA codes, banking alerts, messages)
        - Often used by malware for data harvesting and credential theft
        - Privacy violation through access to personal communications
        """
        printTestInfo("Analyzing Notification Listener Services")
        
        notification_services = self.parser.getNotificationListenerServices()
        jres = []
        
        if len(notification_services) == 0:
            self.logger.info("No notification listener services found")
            self.json_result["Notification Listener Services"] = jres
            return
        ...
        for service in notification_services:
            ...
            # Security warnings
            self.logger.warning(f'Notification listener service found: {service_name}')
            
            if service.exported:
                self.logger.critical(f'Notification listener service {service_name} is exported - HIGH RISK')
            
            if service.enabled is not False:  # None or True
                self.logger.warning(f'Notification listener service {service_name} is enabled by default')
        
        self.json_result["Notification Listener Services"] = jres
        
        # Summary warning
        if len(notification_services) > 0:
            self.logger.warning(f'Found {len(notification_services)} notification listener service(s). '
                               f'These services can access all system notifications and should be carefully reviewed.')
    ...
    def runAllTests(self):
        ...
        # NEW: Add accessibility services, contact providers, and notification listeners analysis
        self.analyzeAccessibilityServices()
        self.analyzeContactDirectoryProviders()
        self.analyzeNotificationListenerServices()
